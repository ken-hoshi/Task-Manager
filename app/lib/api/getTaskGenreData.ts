...
export async function getTaskGenreData(taskIdList: number[]) {
  try {
    ...
    const taskGenreDataArray = taskGenreData.map((task) => {
      ...
      const taskGenreDataFilteredSameTaskGenre = taskGenreData.filter(
        (taskGenre) => taskGenre.task_genre_id === task.task_genre_id
      );

      if (
        taskGenreDataFilteredSameTaskGenre &&
        taskGenreDataFilteredSameTaskGenre.length > 0
      ) {
        assignedUserTaskResults = taskGenreDataFilteredSameTaskGenre.map(
          (task) => {
            return {
              userId: task.assigned_user_id,
              userName: task.users.name,
      ...
      return {
        ...
        numberOfPersons:
          taskGenreDataFilteredSameTaskGenre &&
          taskGenreDataFilteredSameTaskGenre.length > 0
            ? taskGenreDataFilteredSameTaskGenre.length
            : 0,
